...
% Multiple Stages
%	- Identification stage (obtaining WiSARD list from RBNB/database queries)
%	- Command stage (generating commands)
%	- Validation stage (checking that commands valid for each wisard)
%	- Transmission Stage (commands are flushed to the appropriate wisards)
%	- Reconfiguration stage (wisards make requested changes)
%	- Verification stage (wisards generate confirmation packets and sed upstream to datacenter)
...
\subsection{Validating the New Configuration}
...
\subsection{Verifying the New Configuration}
